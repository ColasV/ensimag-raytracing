...
bool draw(char* outputName, scene &myScene) 
{
  ...
  if (rang == 0) {
    imageFile.open(outputName,ios_base::binary);
    if (!imageFile)
      return false; 
    // Ajout du header TGA
  ...
  for (int y = 0; y < myScene.sizey; ++y) {
    if (y % nbM != rang) continue;
    unsigned char results_local[myScene.sizex][3];
	...
	{ 
      ...
      results_local[x][0] = (unsigned char)min(blue*255.0f,255.0f);     
      results_local[x][1] = (unsigned char)min(green*255.0f,255.0f);
      results_local[x][2] = (unsigned char)min(red*255.0f,255.0f);
    ...
    if (rang == 0) {
      for(int i=1; i < nbM; i++) {
	MPI_Status status;
	unsigned char results_from[myScene.sizex][3];
	MPI_Recv(results_from, myScene.sizex * 3, MPI_BYTE,
		 i, 123456, MPI_COMM_WORLD, &status);
	for(int j=0; j < myScene.sizex; j++)
	  imageFile.put((unsigned char)results_from[j][0]).put((unsigned char)results_from[j][1]).put((unsigned char)results_from[j][2]);
      }
    }
    else 
      MPI_Send(results_local, myScene.sizex * 3, MPI_BYTE,
	       0, 123456, MPI_COMM_WORLD);
  }
  return true;
}
